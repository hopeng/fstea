...
//| $Author: hopeng $
//| $Rev: 11M $
//| $Date: 2011-09-28 22:11:13 +1000 (周三, 28 九月 2011) $
//+------------------------------------------------------------------+
...
static int MOMENTUM_MATRIX[4][4] = {
   PERIOD_H1, PERIOD_M15, PERIOD_M5, PERIOD_M1, // 0: mini scalp
   PERIOD_H4, PERIOD_H1, PERIOD_M15, PERIOD_M5, // 1: Scalp trade
   PERIOD_D1, PERIOD_H4, PERIOD_H1, PERIOD_M15, // 2: Day trade
   PERIOD_W1, PERIOD_D1, PERIOD_H4, PERIOD_H1   // 3: Swing trade
};

extern int haTolerance = 1;
extern int tradeStrategy = 0; // choose trade style 0,1,2,3 from momentum matrix
...
int start() {
      // check if the order by OrderNumber still alive, reset OrderNumber to zero if not
      if (OrderNumber > 0 && OrderSelect(OrderNumber, SELECT_BY_TICKET) 
            && OrderCloseTime() > 0) {
...
bool buySignal() {
   ...
   bool tripleScreenUp = stoT0 > STO_MID_VALUE && stoT1 > STO_MID_VALUE;
...
bool sellSignal() {
   ...
   int stoT1 = iStochastic(NULL, timeFrame[1],9, 3, 3, MODE_SMA, 0, MODE_MAIN, 0);
   int stoT2 = iStochastic(NULL, timeFrame[2],9, 3, 3, MODE_SMA, 0, MODE_MAIN, 0);
   bool tripleScreenDown = stoT0 < STO_MID_VALUE && stoT1 < STO_MID_VALUE;
...
int haStatusFor(int timeFrame, int barIndex, string& haStateResultString[]) {
   int result = -1;   
   double haHigh;
   double haLow;
   ...
   ArrayResize(haStateResultString, 1);
   haStateResultString[0] = toString(haOpen) + ":" 
